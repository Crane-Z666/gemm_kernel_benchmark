...
// ===== NEW: Weight Packing Helper for Image2D layout =====
// This function packs weights from logical [OC][IC] to a linear buffer
// that will be used to create a 2D Image for the kernel.
    ...
    int oc_padded, int ic_padded) 
{
    ...
    for (int o = 0; o < oc; ++o) {
        for (int i = 0; i < ic; ++i) {
            ...
            // Map logical (o, i) to image coordinates (x, y) and inner-pixel offsets
            int y = o / 8;
            int x = i / 2;
            ...
            // The kernel reads a pixel (16 bytes) and gets weights for 8 output channels and 2 input channels.
            // The memory layout within the 16-byte pixel is [i_inner][o_inner].
...
void verify_on_cpu(
    ...
    const std::vector<char>& weight_oic,
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::cout << "Benchmarking & Verifying OpenCL Pipeline (using Image2D for weights)" << std::endl;
        ...
        build_opts << "-DFLOAT=float -DFLOAT4=float4 -DFLOAT8=float8 -DFLOAT16=float6 ";
        ...
        const int SRC_C_PACK = 4; // Note: SRC_C is packed by 4 for input, but by 2 for weight image width
        ...
        std::vector<char> host_weight_oic(dst_c * src_c);
        std::vector<float> host_bias(dst_c_padded, 0.f);
        std::vector<float> host_dequant(dst_c_padded * 2, 0.f);
        ...
        for(size_t i = 0; i < host_weight_oic.size(); ++i) host_weight_oic[i] = static_cast<char>((i % 25) - 12);
        ...
        std::cout << "Packing weights for Image2D layout..." << std::endl;
        ...
        cl::Image2D weight_image(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
            weight_image_format,
            src_c_padded / 2, // Image Width
            dst_c_padded / 8, // Image Height
            0, // row_pitch, 0 lets implementation decide
        ...
        kernel_compute.setArg(compute_arg_idx++, weight_image); // Pass the Image2D object
        ...
        verify_on_cpu(cpu_output_nhwc, host_input_nhwc, host_weight_oic, host_dequant, host_bias, bhw, dst_c, src_c);
        
        int error_count = 0;
        double max_diff = 0.0;
        const double tolerance = 1e-3; // Tolerance might need to be slightly higher due to texture filtering/precision differences
